const fs = require('fs');
const path = 'web/src/components/pages/ScanTabs.jsx';
let text = fs.readFileSync(path, 'utf8');

const regex = /export function ArchitectureTab\(\{ ready, session \}\) \{[\s\S]*?\/\/ \-\-\-\-\- Flow \-\-\-\-\-/;

const newCode = export function ArchitectureTab({ ready, session }) {
    ...
    <div className="p-6 max-w-[90rem] mx-auto">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4">
        <div>
          ...
          <p className="text-ink-300 text-[12.5px] mt-0.5">{ctx.lang === 'vi' ? 'Biá»ƒu Ä‘á»“ tÆ°Æ¡ng tÃ¡c 2D vá»›i dÆ° liá»‡u tá»« AST + Gemini.' : 'Interactive 2D force graph powered by AST + Gemini.'}</p>
        </div>
             ...
             {!selectedNode && (
               <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                 <div className="bg-ink-900/80 px-4 py-2 rounded-full border border-ink-700 text-sm text-ink-300 shadow-xl backdrop-blur-sm flex items-center gap-2">
                   ...
                   {ctx.lang === 'vi' ? 'KÃ©o tháº£ vÃ  LÄƒn chuá»™t. Click vÃ o má»™t Node Ä‘á»ƒ bÃ³c tÃ¡ch chi tiáº¿t.' : 'Drag to pan, scroll to zoom. Click a node to view details.'}
                 </div>
               </div>
             )}
          </div>
                 ...
                 <div>
                   ...
                   <div className="text-sm text-amber-100/90 leading-relaxed bg-amber-500/5 p-4 rounded-lg border border-amber-500/10">
                      {(session?.modules || []).find(m => m.name.toLowerCase() === selectedNode.label.toLowerCase())?.purpose[ctx.lang] || 
                       (ctx.lang === 'vi' ? 'Tiáº¿n háº¡nh quÃ©t vÃ  phÃ¢n tÃch báº±ng Gemini Ä‘á»ƒ AI gÃ¡n logic Business Domain cho Node nÃ y.' : 'Analyze with Gemini to generate AI business logic summary for this node.')}
                   </div>
                 </div>
               </>
             ) : (
